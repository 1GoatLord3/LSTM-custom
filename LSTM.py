...
def prepare_data(data, time_step):
    # Use all 11 features: timestamp, open, high, low, close, volume, RSI, EMA50, EMA200, ATR, nATR
    scaler = MinMaxScaler(feature_range=(0, 1))
    scaled_data = scaler.fit_transform(data[['open', 'high', 'low', 'close', 'volume', 'RSI', 'EMA50', 'EMA200', 'ATR', 'nATR']])

    X, y = [], []
    for i in range(len(scaled_data) - time_step - 1):
        X.append(scaled_data[i:(i + time_step), :])  # Use all 11 features for X
        y.append(scaled_data[i + time_step, 3])  # Predict 'close' price (3rd index)
...
for timeframe in timeframes:
    ...
    if data is None:
        print(f"Failed to fetch data for {timeframe} timeframe. Skipping...")
        continue

    # Prepare data
    time_step = 60 if timeframe == '15m' else 24
    feature_count = 10  # We're now using 10 features (excluding the timestamp)
    ...
    # Create a dummy array with the same number of columns as the original scaled data (11 features)
