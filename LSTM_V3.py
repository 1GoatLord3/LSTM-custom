...
def plot_results(original, historical_pred, future_pred, title):
    ...
    if len(historical_pred) > 0:
        plt.plot(range(len(original) - len(historical_pred), len(original)), historical_pred,
                 label='Historical Prediction', color='green')

    if len(future_pred) > 0:
        plt.plot(range(len(original) - 1, len(original) + len(future_pred) - 1), future_pred,
...
# Main execution loop
symbols = ['BTC/USDT']  # Multiple symbols
timeframes = ['1h']
...
while True:
    for symbol in symbols:
        for timeframe in timeframes:
            ...
            # Calculate delta between actual and predicted prices
            deltas = np.abs(original_data[-len(historical_pred):] - historical_pred)
            print(f"Deltas for {symbol} - {timeframe}: {deltas}")
